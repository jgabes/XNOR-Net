...
local opt = lapp[[
   ...
   -m,--model         (default "xnor")   type of model tor train: convnet | mlp | linear
...
if opt.network == '' then
  ...
  elseif opt.model == 'binary' then
      ------------------------------------------------------------
      -- network with binary weights but float transfers 
      ------------------------------------------------------------
      
      model:add(ContConvolution(1,32,5,5,1,1,2,2))       -- 28->28
      model:add(MaxPooling(3,3,2,2))                   -- 28 ->  13
      model:add(ContConvolution(32,64,5,5,1,1,2,2))       --  13 -> 13  
      model:add(MaxPooling(3,3,2,2))                     --  13 ->  6
      model:add(ContConvolution(64,128,3,3,1,1,1,1))      --  6 ->  6
      model:add(ContConvolution(128,128,3,3,1,1,1,1))     --  6 -> 6
      model:add(ContConvolution(128,64,3,3,1,1,1,1))      --  6 -> 6
      model:add(MaxPooling(3,3,2,2))           
      model:add(nn.SpatialDropout(opt.dropout))
      model:add(ContConvolution(64,512,3,3))
      model:add(nn.SpatialDropout(opt.dropout))           
      model:add(ContConvolution(512,512,1,1)) 
      model:add(nn.SpatialConvolution(512, #classes,1,1))
      model:add(nn.Reshape(#classes))
      model:add(nn.LogSoftMax())
 
elseif opt.model == 'xnor' then  
      model:add(nn.SpatialConvolution(1,32,5,5,1,1,2,2))      
      model:add(nn.SpatialBatchNormalization(32,1e-5,false))
      model:add(nn.ReLU(true))
      model:add(MaxPooling(3,3,2,2))

      model:add(BinMaxConvolution(32,64,5,5,1,1,2,2))        
      model:add(BinConvolution(64,128,3,3,1,1,1,1))      
      model:add(BinConvolution(128,128,3,3,1,1,1,1)) 
      model:add(BinMaxConvolution(128,64,3,3,1,1,1,1))        
      model:add(BinConvolution(64,512,3,3))        
      model:add(BinConvolution(512,512,1,1))

      model:add(nn.SpatialBatchNormalization(512,1e-3,false))
      model:add(nn.ReLU(true))
      model:add(nn.SpatialConvolution(512, #classes,1,1))
